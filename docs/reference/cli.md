...
`--strict` is a global flag, disabled by default. When enabled, it enforces the language requirements of the installed compiler by treating migration warnings as errors:
...
`--go-vine-version` overrides the Vine requirement written into generated module output. The value must be a `v`-prefixed semantic version no lower than skelc's minimum supported Vine version, which may be lower than its default. `skelc version` reports both values.
...
With `--api`, use `--go-vrpc-version` to override vRPC v0.12.0; `--go-vine-version` does not apply. A module prefix derives `example.com/gen/shop/orderapi` for domain `shop.order`, and foreign domains use their corresponding API modules. Backend Go output requires Vine v0.15.4 or later; `skelc version` reports `minimumVineVersion` and `minimumApiServiceVineVersion`.
