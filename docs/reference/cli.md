...
`--go-vine-version` overrides the Vine requirement written into generated module output. The value must be a complete `v`-prefixed semantic version, such as `v0.15.7`, that is compatible with the Go module path and no lower than skelc's minimum supported Vine version. `skelc version` reports that minimum and the default it writes when the flag is omitted.
