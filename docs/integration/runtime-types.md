...
Go collection nullability and the encoding contract are described in
[Go Generation](/docs/generation/go#collection-encoding-and-validation).
The `binary` scalar keeps its byte encoding and is not a `list<T>`.

## Domain Schema Registry

Each generated package registers its domain schema, and an application reads the
registered schemas with:

```go
skel.RegisteredDomainSchemas()
```

The result is sorted stably by `Domain`, which keeps application registration, test
snapshots, and log comparisons deterministic.

A schema generated by a compiler older than `skel.MinSkelcVersion()` fails during
application startup.
