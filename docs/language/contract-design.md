...
Split a domain when two groups of declarations have independent release decisions and consumers. Do not split merely because the implementation runs in two processes: Vine can move runtime topology without changing the Skel owner.
...
The public type is designed for consumers. The internal type can follow storage and implementation needs.

## Share Contracts, Not Source Trees

A producer exports reduced Skel with `skelc gen skel --pub`. Consumers map that output through `--skel-import`. They should not parse the producer's private contract directory or rely on a neighboring checkout.

This separation makes an accidental private-to-public reference a compiler error and gives the exported contract its own versionable artifact.
...
Adding a nullable field is often easier to adopt, but still changes generated APIs. Regenerate each target and review the diff rather than inferring compatibility from the `.skel` edit alone.

## Keep Implementation Details Out

Database rows, cache layouts, internal state machines, retry policies, route paths, and broker configuration should rarely appear as public contract data. Model the stable fact or capability that consumers use.

A good boundary normally has:
