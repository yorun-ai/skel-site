...
A Skel file describes a boundary that must hold true across implementations, generated languages, and runtime wiring. The compiler resolves every declaration into one domain model before any generator runs.
...
Don't name a domain after a repository, process, team sprint, or current deployment shape. Those change more often than the boundary they own.
...
6. Run `skelc format` and `skelc check` — generators reject invalid input, so catching issues here gives clearer diagnostics than a failed generation.
