...
manifest. Unmarked files are left alone, while marked files that are no longer
generated are removed. If a handwritten file lands on a path produced by the
current generation, the next run will overwrite it.
