...
generated and handwritten files can share a directory. Unmarked files are left
alone. Regeneration removes previously generated files that still carry the
marker but are no longer needed. If a handwritten file lands on a path produced
by the current generation, the next run will overwrite it.
