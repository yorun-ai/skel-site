...
The development generator now requires and defaults to Vine v0.15.0 for
`index(n)` and `noTrim` tag support. Update the dependencies yourself when
generating into an existing module.
...
## Argument Tags (Unreleased)

The development generator writes argument positions as `skel:"index(0)"`
...
This output requires Vine v0.15.0 or later. Generated modules default to
v0.15.0; older versions cannot register arguments with the new tags.
