...
当前生成的 Go 契约需要 Vine v0.20.2 或更高版本。`skelc version` 会同时显示最低支持版本和默认 Vine 版本；生成 Go module 时，默认版本会写入输出的 `go.mod`，`--go-vine-version` 可以选择其它版本，但不能低于最低支持版本。更高版本不保证一定兼容，因此升级时应固定具体版本、重新生成并运行应用测试。
