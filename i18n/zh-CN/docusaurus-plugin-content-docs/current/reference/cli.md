...
- `gen go` 写入已有模块，支持 `--api`、`--pub`、`--skel-import` 和 `--go-import`，不创建 `go.mod`
- `gen go-module --api` 生成 Portal Go 客户端，依赖 `go.yorun.ai/vrpc` v0.12.0 或更高版本，可用 `--go-vrpc-version` 指定；`--api` 与 `--pub` 互斥
- API 模块按 prefix 和 domain 推导为 `…/orderapi`；显式 `--go-module` 优先，跨领域 API 类型引用对应的 `xxxapi` 包
- 所有生成文件顶部附近都带有 `Code generated by skelc. DO NOT EDIT.`
  所有权标记；无标记文件不会被删除。重新生成时，会删除带有生成标记、但已不再需要的文件
...
- 输出 API 服务客户端、所需数据依赖，以及显式 `pub data/enum` 及其依赖
- 没有 API 服务的领域也允许生成纯类型 API 包
- 旧服务包含客户端准入规则时，给 warning 并兼容生成 API 客户端
...
- `gen skel` 必须传 `--pub`
- 输出保留 `pub` 契约及其完整数据依赖；隐式数据依赖保留原有可见性标记
