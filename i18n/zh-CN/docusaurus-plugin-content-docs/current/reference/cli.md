...
Go 集成可在解析和编译时传入 `skelc.Input{SkelIn: "./skel", Strict: true}`。升级 skelc 后，新增的迁移规则也会纳入严格检查。
...
输入通过 `--skel-in` 指定，也支持全局 `--strict`。语法分析会在声明、block 成员、右花括号和 decorator
...
兼容性分析复用 `skelc schema diff` 的规范化投影和影响分级规则。默认会把 domain 的源文件或目录与 Git `HEAD` 比较，客户端也可以提供显式 baseline 源码路径。`BREAKING`、`DANGEROUS` 和可选的 `COMPATIBLE` 变化分别以 warning、information 和 hint 诊断展示。
...
`--go-vine-version` 覆盖写入生成 module 的 Vine 依赖版本。必须是完整的、带 `v` 前缀的语义版本（例如 `v0.19.0`），与 Go module 路径兼容，且不低于最低支持版本。`skelc version` 会报告该最低版本，以及未指定该参数时写入的默认版本。
...
`--api` 生成 Portal 客户端，可用 `--go-vrpc-version` 覆盖默认的 vRPC v0.12.0，不接受 `--go-vine-version`。后端 Go 输出要求 Vine v0.19.0 或更高版本；`skelc version` 的 `minimumVineVersion` 会报告此要求。module prefix 会把 domain `shop.order` 推导为 `example.com/gen/shop/orderapi`，跨领域 API 类型引用对应的 `xxxapi` 包。
...
- 已声明的 `web` 挂载路径会以 `MountPath` 写入生成的 Web spec 和 runtime schema，并要求 Vine v0.19.0 或更高版本
